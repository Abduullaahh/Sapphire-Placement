...
interface Service {
  ...
  title: string;
  description: string;
  icon: keyof typeof icons; // Ensure the icon key is valid
}
...
export default function ServiceDetail() {
  ...
  const [service, setService] = useState<Service | null>(null); // Type the state

  useEffect(() => {
    if (typeof window !== 'undefined' && slug) {
      const storedService = localStorage.getItem('selectedService');
      if (storedService) {
        const parsedService: Service = JSON.parse(storedService); // Type the parsed service
    ...
    <Layout>
      <div className="bg-white">
        <div className="max-w-3xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
          ...
          >
            <motion.div className="w-16 h-16" animate="animate" variants={iconRotate}>
              <Icon className="h-16 w-16 text-[#007BB8] mb-6" />
            </motion.div>
            <h1 className="text-3xl font-extrabold text-[#007BB8] sm:text-4xl ml-4">
              {service.title}
            </h1>
          </motion.div>

          <p className="mt-6 text-xl text-gray-600 text-left">{service.description}</p>

          <motion.div className="mt-12" initial="initial" animate="animate" variants={fadeInUp}>
              ...
              <ul className="space-y-4">
                {[
                  ...
                  >
                    ...
                    <span className="hover:text-[#007BB8] transition-colors duration-300">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <motion.div className="mt-12" initial="initial" animate="animate" variants={fadeInUp}>
            <h2 className="text-2xl font-bold text-[#007BB8] mb-6 relative">Our Process</h2>
            <div className="relative pl-6">
              <div className="absolute left-0 top-0 w-1 h-full bg-[#007BB8] rounded-full opacity-20"></div>
          ...
          <motion.div className="mt-12 text-center" initial="initial" animate="animate" variants={fadeInUp}>
            <motion.button
